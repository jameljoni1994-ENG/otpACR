...
وتنفّذ حزمتنا $B_{k}$ عبر ضربات متجه مضغوطة فقط وتحل المسألة الفرعية التكعيبية بنفس إجراء كريلوف التكيّفي للمحور~2، متجنّبة بناء $B_{k}\in\R^{n\times n}$ صراحة عند $n\sim 10^{4}$.
\end{proposition}
...
أزواج ذاكرة L-BFGS داخل النموذج التكعيبي عبر ضربات $B_{k}v$ بلا مصفوفة كاملة وحل كريلوف للمسألة الفرعية؛ مقارنة مع L-BFGS الصرف.
...
\begin{itemize}[leftmargin=*]
  \item \textbf{النظرية مقابل التطبيق.} المبرهنات~\ref{thm:cr-rate}--\ref{thm:krylov-adapt} تبرّر ARC--Krylov التكيّفي كطريقة واسعة النطاق تحافظ على التعقيد. وعلى النماذج الخطية المعممة غالبًا يفوز L-BFGS بالزمن لأن كل تكرار أرخص من فضاء كريلوف حتى المتواضع (جدول~\ref{tab:full}).
  \item \textbf{التعميم.} تحسين $\norm{\grad f}$ ليس وكيلًا لدقة الاختبار؛ مراقبة التحقق ضرورية (جداول~\ref{tab:softmax}--\ref{tab:full}).
  \item \textbf{متى يُستخدم CR.} يُفضَّل ARC--Krylov عند توفر HVP وأهمية جودة الحل. ويُفضَّل L-BFGS+ES للتدريب الروتيني متعدد التصنيفات؛ وQN-CR بلا كثافة حل وسط منافس على MNIST الكامل.
  \item \textbf{القيود.} طرق الموتر استكشافية؛ تسريع الرسم الموزّع متواضع بسبب عبء العمليات في Python؛ ويبقى اتصال MPI الحقيقي وجداول $(M,\ell_{2})$ المشتركة أبعد من المسارات ذات التهيئة الدافئة عملًا مفتوحًا.
\end{itemize}
...
وتغطي اختبارات الوحدة $34$ اختبارًا ناجحًا.
