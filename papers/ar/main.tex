...
تجريبيًا، على دوال تحليلية وMNIST الحقيقي (لوجستي ثنائي وSoftmax بعشرة أصناف بـ $7840$ معاملًا)، يتفوّق ARC--Krylov في $f$ و$\norm{\grad f}$، بينما L-BFGS الأسرع غالبًا؛ ورفع الإيقاف المبكر دقة الاختبار من نحو $87\%$ إلى نحو $90\%$ على عيّنة جزئية، وإلى نحو $91\%$ على مجموعة التدريب الكاملة $\approx 60$ ألفًا.
ونوّحد كذلك كريلوف غير الدقيق التكيّفي داخل ARC، ونستبدل خطوات شبه-نيوتن الكثيفة بضربات $B_{k}v$ بلا مصفوفة كاملة، ونضيف اختبار نسبة بأسلوب ARC للنسخ العشوائية مع مسار تنظيم $\ell_{2}$ تحت الإيقاف المبكر.
تصاحب الدراسة حزمة بايثون قابلة لإعادة الإنتاج (\textenglish{\texttt{cubic\_reg}}~$0.2$).
\end{abstract}
...
المُصغِّر العالمي لـ~\eqref{eq:cubic-model} يؤول إلى معادلة سيكولارية بعد التحليل الذاتي لـ $\hess f(x_{k})$~\cite{conn2000trust,cartis2011arc}. التكلفة $O(n^{3})$ والذاكرة $O(n^{2})$ تمنع التطبيقات واسعة النطاق.

\paragraph{المساهمات.}
\begin{enumerate}[leftmargin=*,itemsep=2pt]
  \item \textbf{نظرية.} تطوير مكتفٍ ذاتيًا لتعقيد CR (مبرهنة~\ref{thm:cr-rate})، والتعجيل (مبرهنة~\ref{thm:acc-cr})، وARC (مبرهنة~\ref{thm:arc})، وشروط عدم الدقة (مبرهنة~\ref{thm:inexact})، وجدولة كريلوف التكيّفية (مبرهنة~\ref{thm:krylov-adapt})، ونقل حساب المتبقي لخطوات L-BFGS التكعيبية بلا كثافة مع خطأ هيسيان صريح (مبرهنة~\ref{thm:mf-qn}).
  \item \textbf{خوارزميات.} نحدد خوارزميتي CR الدقيق وARC--Krylov كما في حزمة \textenglish{\texttt{cubic\_reg}}.
  \item \textbf{تجارب.} عشر مراحل: معدلات تحليلية، توسع كريلوف ($n\le 10^{4}$)، سياسات $\theta_{k}$، ARC موزّع، وMNIST مع مسح معاملات وإيقاف مبكر، ثم MNIST الكامل ($\approx 60$ ألفًا)، ومسار $\ell_{2}$، وتوسع الرسم التقريبي عند $n\in\{2{\cdot}10^{3},8{\cdot}10^{3},2{\cdot}10^{4}\}$.
  \item \textbf{برمجيات.} مسار ARC--Krylov الكانوني يستخدم نفس حل المسألة الفرعية غير الدقيق التكيّفي كما في أغلفة التدريب؛ وQN التكعيبي بلا مصفوفة كثيفة افتراضيًا؛ وCR العشوائي يكيّف $M$ باختبار نسبة.
  \item \textbf{توصيات.} ARC--Krylov لجودة الحل؛ L-BFGS (أو ARC) مع إيقاف مبكر للتعميم تحت ميزانية زمنية؛ ومسار $\ell_{2}$ اختياري عند حساسية التنظيم.
\end{enumerate}
...
\label{def:subopt}
المتجه $s^{\star}$ مُصغِّر عالمي لـ $m_{k}$ إذا وفقط إذا وُجد $\lambda^{\star}\ge 0$ بحيث~\cite{conn2000trust}
\begin{align}
...
إذا وُلِّدت $(x_{k})$ بتصغير دقيق لـ~\eqref{eq:cubic-model} بثابت $M$، فإن
\begin{equation}
\label{eq:cr-rate}
f(x_{k})-f_{\star}
\le
...
\begin{proof}
ليكن $g_{k}=\grad f(x_{k})$ و$H_{k}=\hess f(x_{k})$ و$s_{k}$ مُصغِّر $m_{k}$.
...
\end{equation}
تُقبل الخطوة إذا $\rho_{k}\ge\eta_{1}\in(0,1)$؛ ويُصغَّر $M_{k}$ إذا $\rho_{k}\ge\eta_{2}>\eta_{1}$ ويُكبَّر إذا $\rho_{k}<\eta_{1}$.
\end{definition}
...
\begin{proof}
المتبقي~\eqref{eq:resid} يجعل نقصان النموذج يختلف عن حالة المُصغِّر الدقيق بحد $O(\theta_{k}\norm{g_{k}}\norm{s_{k}})$.
...
ثم تُطبَّق حجة الشحن نفسها كما في المبرهنة~\ref{thm:arc}.
\end{proof}
...
\begin{proof}
عند $m=n$ يستعيد لانتشوز الأساس الذاتي الكامل ويتطابق الحل المسقط مع المُصغِّر الدقيق فيكون المتبقي صفرًا.
...
عمليًا، L-BFGS الصرف شديد المنافسة على النماذج الخطية المعممة.
\end{proposition}

\begin{theorem}[خطوة L-BFGS التكعيبية بلا مصفوفة كثيفة]
\label{thm:mf-qn}
لتكن $B_{k}$ هيسيانية L-BFGS محدودة الذاكرة من $\mu$ أزواج انحناء على الأكثر، تُطبَّق فقط عبر الضرب المضغوط $v\mapsto B_{k}v$ بتكلفة $O(\mu n+\mu^{2})$~\cite{byrd1994representationsations}.
اعتبر النموذج التكعيبي الذي يستبدل $\hess f(x_{k})$ بـ $B_{k}$، وحُلّه بإجراء لانتشوز التكيّفي في المبرهنة~\ref{thm:krylov-adapt} مع استبدال HVP بضربات $B_{k}v$، فينتج $(s_{k},\lambda_{k})$ يحقق
\begin{equation}
\label{eq:resid-b}
\norm{(B_{k}+\lambda_{k}I)s_{k}+g_{k}}
\le
\theta_{k}\norm{g_{k}}.
\end{equation}
عندئذٍ:
\begin{enumerate}[leftmargin=*,itemsep=1pt]
  \item \textbf{(تكافؤ تام على النموذج.)}
  عندما يبلغ بُعد كريلوف $n$، تتطابق الخطوة مع المُصغِّر العالمي لنموذج $B_{k}$ التكعيبي؛ ويتحقق~\eqref{eq:resid-b} بمتبقٍ صفر.
  \item \textbf{(تعقيد المسألة الفرعية.)}
  لبُعد داخلي $\le m$ تكون التكلفة
  $O\bigl(m(\mu n+\mu^{2})+m^{2}n+m^{3}\bigr)$ زمنًا و$O(\mu n)$ ذاكرة إضافية، دون بناء $B_{k}\in\R^{n\times n}$.
  \item \textbf{(نقل تعقيد ARC تحت خطأ الهيسيان.)}
  إذا كان $\norm{B_{k}-\hess f(x_{k})}\le\xi_{k}$ وبقي $\theta_{k}+\xi_{k}/\lambda_{\min}(B_{k}+\lambda_{k}I)$ تسلسل عدم دقة قابل للجمع بمعنى المبرهنة~\ref{thm:inexact}، فإن تعقيد ARC من رتبة $O(\varepsilon^{-3/2})$ في المبرهنة~\ref{thm:arc} ينطبق على الهدف الحقيقي $f$.
\end{enumerate}
\end{theorem}

\begin{proof}
(١) كما في المبرهنة~\ref{thm:krylov-adapt} مع استبدال المؤثر $H$ بـ $B_{k}$: لانتشوز الكامل يستعيد الأساس الذاتي لـ $B_{k}$ في الحساب الدقيق.
(٢) مباشرة من الضرب المضغوط والقضية~\ref{prop:cost}.
لـ (٣) اكتب
\[
\norm{(H_{k}+\lambda_{k}I)s_{k}+g_{k}}
\le
\norm{(B_{k}+\lambda_{k}I)s_{k}+g_{k}}
+
\norm{(H_{k}-B_{k})s_{k}}.
\]
الحد الأول $\le\theta_{k}\norm{g_{k}}$ من~\eqref{eq:resid-b}، والثاني $\le\xi_{k}\norm{s_{k}}$.
وعلى خطوات ARC الناجحة $\lambda_{k}=\tfrac{M_{k}}{2}\norm{s_{k}}$ وتعطي علاقة الأمثلية التكعيبية $\norm{s_{k}}=O(\norm{g_{k}}/\lambda_{\min}(B_{k}+\lambda_{k}I))$ حتى ثوابت مطلقة~\cite{cartis2011arc}، فيظهر تسامح متبقٍ فعّال من رتبة $\theta_{k}+C\xi_{k}/\lambda_{\min}(\cdot)$.
ومتى حقق هذا التسلسل فرضيات المبرهنة~\ref{thm:inexact} ينتقل نفس جدال الشحن إلى التعقيد الخارجي.
\end{proof}

\begin{remark}
\label{rem:mf-qn-practice}
عمليًا نأخذ $\theta_{k}=\theta_{0}/(k+1)^{p}$ كما في ARC--Krylov، ونبقي $\mu\approx 10$--$20$، ونكيّف $M_{k}$ اختياريًا باختبار نسبة على نموذج $B_{k}$.
وعندما يكون تقريب $B_{k}$ لـ $H_{k}$ ضعيفًا ( $\xi_{k}$ كبير)، قد يفشل البند (٣) فيسلك الأسلوب سلوك عولمة لـ L-BFGS لا أسلوب نيوتن حافظ للتعقيد --- بما يتسق مع القضية~\ref{prop:lbfgs}.
\end{remark}
...
تدرجات/هيسيانات دفعات مع اختبار نسبة بأسلوب ARC على $M$ وتقييم كامل نادر للتسجيل/التوقف؛ مقارنة زمنية مع SGD/Adam على اللوجستي/MNIST.
...
أزواج ذاكرة L-BFGS داخل النموذج التكعيبي عبر ضربات $B_{k}v$ بلا مصفوفة كاملة وحل كريلوف للمسألة الفرعية (مبرهنة~\ref{thm:mf-qn})؛ مقارنة مع L-BFGS الصرف و$B_{k}$ الكثيفة.
...
السكربتات: \textenglish{\texttt{experiments/run\_phase\{1--10\}.py}}.
اختبارات الوحدة: $34$ ناجحة.
...
\subsection{MNIST الكامل ومسار $\ell_{2}$ وتوسع الرسم}
توحّد المرحلة~10 مسار كريلوف غير الدقيق التكيّفي داخل \textenglish{\texttt{arc.minimize(mode="krylov")}}، وتفعّل CR شبه-نيوتن بلا مصفوفة كثيفة افتراضيًا، وتقيّم أنظمة بيانات أكبر (\textenglish{\texttt{experiments/run\_phase10.py}}).

\paragraph{MNIST الكامل ($\approx 60$ ألفًا).}
تقسيم تدريب/تحقق/اختبار $\approx 38400/9600/12000$، بُعد $7840$، صبر $6$.

\begin{table}[t]
\centering
\caption{Softmax على MNIST الكامل مع إيقاف مبكر (المرحلة~10).}
\label{tab:full}
\begin{tabular}{lrrr}
\toprule
الطريقة & دقة الاختبار & الزمن (ث) & ملاحظات \\
\midrule
ARC--Krylov + ES & \textbf{0.913} & 52.9 & $\theta_{k}$ تكيّفي، HVP \\
L-BFGS + ES & 0.910 & \textbf{3.7} & SciPy L-BFGS-B \\
QN-CR (بلا كثافة) & 0.910 & 2.6 & $B_{k}v$ مضغوط + كريلوف \\
\bottomrule
\end{tabular}
\end{table}

على العيّنة المتوسطة ($\approx 12$ ألفًا) يصل ARC--ES إلى $\approx 0.921$ مقابل $\approx 0.914$ لـ L-BFGS--ES، بما يؤكد استقرار الترتيب تحت الإيقاف المبكر.

\paragraph{مسار تنظيم $\ell_{2}$.}
إيقاف مبكر مع تهيئة دافئة على $\lambda\in\{10^{-2},3{\cdot}10^{-3},10^{-3},3{\cdot}10^{-4},10^{-4}\}$ يعطي أفضل نماذج تحقق بدقة اختبار $\approx 0.898$ (مسار ARC) و$\approx 0.892$ (مسار L-BFGS) على عيّنة $8$ آلاف --- مفيد عند عدم يقين اختيار $\ell_{2}$ واحد.

\paragraph{توسع الرسم--كريلوف.}
يسجّل ARC--Krylov بالرسم بلا مصفوفة كثيفة على تربيعيات تحليلية أزمنة تقريبًا $0.02$ / $0.07$ / $0.33$ ث عند $n=2{\cdot}10^{3}/8{\cdot}10^{3}/2{\cdot}10^{4}$ (رتبة رسم $20$)، بما يتسق مع نمو شبه خطي لتكلفة HVP.

\begin{figure}[t]
\centering
\includegraphics[width=0.95\linewidth]{25_phase10_mnist.png}
\caption{ملخص مقارنة MNIST في المرحلة~10.}
\label{fig:p10mnist}
\end{figure}
...
\caption{مسار $\ell_{2}$ مع إيقاف مبكر (دقة التحقق على طول المسار).}
\label{fig:l2path}
\end{figure}

\begin{figure}[t]
\centering
\includegraphics[width=0.85\linewidth]{27_phase10_sketch_scale.png}
\caption{توسع زمن الرسم--كريلوف مقابل البُعد $n$.}
\label{fig:p10sketch}
\end{figure}

\subsection{QN-CR بلا كثافة (مبرهنة~\ref{thm:mf-qn})}
تتحقق المرحلة~11 من تطابق الضرب المضغوط لـ BFGS مع $B_{k}$ الكثيفة ومن توسع QN-CR بلا كثافة (\textenglish{\texttt{experiments/run\_phase11.py}}).
على أزواج انحناء عشوائية يتفق \textenglish{matvec} مع $B_{k}v$ حتى دقة الآلة.
وعلى تربيعية بـ $n=40$ يصل المساران إلى نفس الأمثل تقريبًا.
وتبقى الطريقة عملية حتى $n=4{\cdot}10^{3}$ دون تخزين $B_{k}$.

\begin{figure}[t]
\centering
\includegraphics[width=0.85\linewidth]{28_phase11_qn_scale.png}
\caption{توسع زمن QN-CR بلا كثافة.}
\label{fig:p11scale}
\end{figure}

\begin{figure}[t]
\centering
\includegraphics[width=0.85\linewidth]{29_phase11_bfgs_ops.png}
\caption{تكلفة \textenglish{\texttt{to\_dense}} مقابل \textenglish{matvec}.}
\label{fig:p11ops}
\end{figure}
...
\begin{itemize}[leftmargin=*]
  \item \textbf{النظرية مقابل التطبيق.} المبرهنات~\ref{thm:cr-rate}--\ref{thm:krylov-adapt} و~\ref{thm:mf-qn} تبرّر ARC--Krylov وQN-CR بلا كثافة كطرق واسعة النطاق واعية بالتعقيد. وعلى النماذج الخطية المعممة غالبًا يفوز L-BFGS بالزمن (جدول~\ref{tab:full}).
  \item \textbf{التعميم.} تحسين $\norm{\grad f}$ ليس وكيلًا لدقة الاختبار؛ مراقبة التحقق ضرورية (جداول~\ref{tab:softmax}--\ref{tab:full}).
  \item \textbf{متى يُستخدم CR.} يُفضَّل ARC--Krylov عند توفر HVP وأهمية جودة الحل. ويُفضَّل L-BFGS+ES للتدريب الروتيني؛ وQN-CR بلا كثافة هو الهجين القابل للتوسع في المبرهنة~\ref{thm:mf-qn}.
  \item \textbf{القيود.} طرق الموتر استكشافية؛ تسريع الرسم الموزّع متواضع؛ ويصعب اعتماد فرضية خطأ الهيسيان في المبرهنة~\ref{thm:mf-qn}(٣) مسبقًا لـ L-BFGS.
\end{itemize}

\section{البرمجيات}
توفر حزمة \textenglish{\texttt{cubic\_reg}} (الإصدار~$0.2$) الحالّات والمسائل والمقاييس وأغلفة التدريب (\textenglish{\texttt{arc\_krylov\_early\_stop}}، \textenglish{\texttt{l2\_path\_early\_stop}}) والرسوم وسكربتات المراحل.
وتغطي اختبارات الوحدة $36$ اختبارًا ناجحًا.

\section{خاتمة}
قدّمنا عرضًا نظريًا موحّدًا للتنظيم التكعيبي --- المعدلات العالمية، والتعجيل، وتعقيد ARC، وشروط كريلوف غير الدقيقة التكيّفية --- مع تقييم تجريبي يشمل MNIST حتى المقياس الكامل.
يظهر ARC--Krylov التكيّفي وL-BFGS مع الإيقاف المبكر كأداتين عمليتين متكاملتين؛ ويقرّب QN التكعيبي بلا كثافة ومسارات $\ell_{2}$ الفجوة بين النظرية والممارسة عند أبعاد كبيرة.
العمل المستقبلي: جداول $(M,\ell_{2})$ المشتركة بضمانات أقوى، وخلفيات HPC/MPI للاتصال بالرسم.
...
\label{app:exp}
تُسجَّل المراحل 1--10 بصيغة JSON تحت \textenglish{\texttt{experiments/results/}} (بما فيها \textenglish{\texttt{phase10.json}}).
التجارب الموزّعة تستخدم مجمع \textenglish{\texttt{multiprocessing}} ثابتًا لتجنب عبء الإنشاء في Windows.
وتُنفَّذ HVP لـ Softmax بدقة عبر جاكوبي التحويل الخطي $W\in\R^{10\times 784}$ دون بناء هيسيان $7840\times 7840$.
وتسمح كتابة الدمج في المرحلة~10 بإعادة تشغيل جزئية (\textenglish{\texttt{--skip-mnist}}/\textenglish{\texttt{--skip-l2}}/\textenglish{\texttt{--skip-sketch}}) دون مسح الأقسام الشقيقة.

\begin{thebibliography}{99}
\bibitem{byrd1994representationsations}
R.~H.~Byrd, J.~Nocedal, and R.~B.~Schnabel.
Representations of quasi-Newton matrices and their use in limited memory methods.
\textit{Mathematical Programming}, 63:129--156, 1994.
